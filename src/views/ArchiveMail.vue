                    ...
                    <div class="email-actions d-flex">
                        <span title="Show">
                            <EyeIcon style="cursor: pointer;" class="h-5 w-5 mx-2"
                                @click.stop="$router.push({ name: 'EmailDetails', params: { id: email.id } })" />
                        </span>
...
export default {
    ...
    methods: {
        ...
        async getArchiveEmails() {
            try {
                this.loading = true; // Show loader
                const response = await axios.get(`${apiUrl}emailsArchive`);
        ...
        async deleteEmail(emailId) {
            try {
                ...
                toastr.success('Successfully Email Deleted.');
                this.getArchiveEmails();
            } catch (error) {
